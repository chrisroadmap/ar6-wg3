...
"""Run AR6 database with calibrated scenarios using restart"""
...
for batch_start in tqdm(range(batch_num * batch_size, len(vetted), batch_size), total=int(np.ceil(len(vetted)/batch_size))):
    ...
    for iscen in range(batch_start, batch_end):
        ...
        for fairname, rcmipname in fair_to_rcmip.items():
            f.emissions.loc[dict(specie=fairname, scenario=f"{vetted.loc[iscen, 'Model']}___{vetted.loc[iscen, 'Scenario']}")] = filt.loc[
